...
import { assetStatusLabels, assetTitle, labelOf, rentalStatusLabels } from '../utils/labels';
...
export default function AssetPage() {
  ...
  const rent = async (assetNo) => {
    ...
    try {
      ...
      await notify({
        title: '대여 신청 완료',
        message: '관리자 승인 후 기자재 대여가 확정됩니다.',
        type: 'success'
      });
      await load();
    } catch (error) {
      await notify({
        title: '대여 신청 실패',
        message: error?.response?.data?.message || '잠시 후 다시 시도해 주세요.',
        type: 'danger'
      });
    } finally {
      setRentingNo(null);
                ...
                <div className="service-row asset-service-row" key={asset.assetNo}>
                  <Link to={`/assets/${asset.assetNo}`} className="asset-row-link">
                    ...
                    <div>
                      ...
                      <span>{asset.category || '기타'} · {asset.description || '등록된 설명이 없습니다.'}</span>
                    </div>
                  </Link>
                  <div className="service-row__actions">
                    <span className={`status-pill ${statusClass(asset.status)}`}>{labelOf(assetStatusLabels, asset.status)}</span>
              ...
              {myRentals.map((rental) => (
                <div className="service-row" key={rental.rentalNo}>
                  <div>
                    ...
                    <span>반납 예정 · {formatDate(rental.returnDueDate)}</span>
                    <StatusTimeline steps={['REQUESTED', 'APPROVED', 'RETURNED']} current={rental.rentalStatus} />
                  </div>
                  <span className={`status-pill ${statusClass(rental.rentalStatus)}`}>{labelOf(rentalStatusLabels, rental.rentalStatus)}</span>
                </div>
              ))}
