    ...
    <div className="workspace-page">
      <section className="workspace-hero">
        <div className="workspace-hero__copy">
          ...
          <h1>관리자 대시보드</h1>
          <p>공지 등록, 시설 신고 처리, 기자재 승인, 공간 예약을 한 화면에서 운영합니다.</p>
        </div>
        <div className="workspace-hero__aside"><span>오늘 예약</span><strong>{stats.todayReservations || 0}건</strong></div>
      </section>

      <section className="workspace-stats">
        <div className="workspace-stat"><span>전체 회원 수</span><strong>{stats.totalUsers || 0}</strong><small>등록 사용자</small></div>
        <div className="workspace-stat"><span>처리 대기 신고</span><strong>{stats.pendingReports || 0}</strong><small>관리자 확인 필요</small></div>
        <div className="workspace-stat"><span>대여 신청 수</span><strong>{stats.requestedRentals || 0}</strong><small>승인 대기</small></div>
        <div className="workspace-stat"><span>오늘 예약 수</span><strong>{stats.todayReservations || 0}</strong><small>공간 이용 일정</small></div>
      </section>

      <div className="workspace-grid two">
        <section className="workspace-card">
          <div className="workspace-card__head"><div><h2>최근 공지</h2><p>최근 등록된 운영 공지입니다.</p></div><Link className="secondary-button" to="/admin/notices">공지 관리</Link></div>
          <div className="workspace-list">
            {(stats.recentNotices || []).map((item) => (
              <div className="workspace-row" key={item.noticeNo}>
                <div className="workspace-row__main"><strong>{item.title}</strong><span>{item.category || '기타'} · 조회수 {item.viewCount || 0}</span></div>
        ...
        <section className="workspace-card">
          <div className="workspace-card__head"><div><h2>최근 신고</h2><p>빠르게 처리해야 할 시설 신고입니다.</p></div><Link className="secondary-button" to="/admin/reports">신고 관리</Link></div>
          <div className="workspace-list">
