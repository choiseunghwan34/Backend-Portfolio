  ...
  return (
    <div className="workspace-page">
      <section className="workspace-hero">
        <div className="workspace-hero__copy"><span className="workspace-label">NOTICE ADMIN</span><h1>공지 관리</h1><p>중요 공지와 운영 안내를 등록하고 수정합니다.</p></div>
        <div className="workspace-hero__aside"><span>등록 공지</span><strong>{items.length}건</strong></div>
      </section>
      <div className="workspace-grid two">
        <section className="workspace-card">
          <div className="workspace-card__head"><div><h2>{editingId ? '공지 수정' : '공지 등록'}</h2><p>사용자에게 노출될 공지 내용을 작성합니다.</p></div></div>
        ...
        <section className="workspace-card">
          <div className="workspace-card__head"><div><h2>공지 목록</h2><p>등록된 공지를 수정하거나 삭제합니다.</p></div></div>
          <div className="workspace-list">
            {items.map((item) => (
              <div className="workspace-row" key={item.noticeNo}>
                <div className="workspace-row__main"><strong>{item.title}</strong><span>{item.category || '일반'} · 조회수 {item.viewCount || 0}</span></div>
                <div className="workspace-row__actions">
                  ...
                  <button className="secondary-button" onClick={() => { setEditingId(item.noticeNo); setForm({ title: item.title, content: item.content, category: item.category || '', importantYn: Boolean(item.importantYn) }); }}>수정</button>
